...
namespace UnitOfWorkApplication.API.Services
{
    ...
    public static class TimeSensitivePassCode
    {
        ...
        public static IList<string> GetListOfOTPs(string base32EncodedSecret)
        {
            ...
            ///Get the interval upto which the OTP would be valid for a user
            var counters = Int32.Parse(RideMeResources.OTPExpirationDuration) / 30;

            for (int i = -1 * counters / 2; i <= counters / 2; i++)
            {

                otps.Add(GetHotp(base32EncodedSecret, counter + i)); // previous OTP
            }
